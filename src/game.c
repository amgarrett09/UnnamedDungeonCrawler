...
static void render_tile_map(i32 *image_buffer, i32 *tile_map, void *tile_set,
...
void game_initialize_memory(Memory *memory, i32 dt)
{
        ...
        size_t tile_load_result =
            load_bitmap("resources/tile_set.bmp", (void *)next_load_location);
        if (tile_load_result) {
        ...
        if (keys & KEYMASK_UP || keys & KEYMASK_DOWN) {
                ...
                if (new_tile_y >= 0 && new_tile_y < SCREEN_HEIGHT_TILES &&
                    tile_x >= 0 && tile_x < SCREEN_WIDTH_TILES) {
                        is_not_colliding =
                            !current_collision_map[new_tile_y *
                                                       SCREEN_WIDTH_TILES +
                                                   tile_x];
                }
                ...
                if (new_tile_x >= 0 && new_tile_x < SCREEN_WIDTH_TILES &&
                    tile_y >= 0 && tile_y < SCREEN_HEIGHT_TILES) {
                        is_not_colliding =
                            !current_collision_map[tile_y * SCREEN_WIDTH_TILES +
                                                   new_tile_x];
                }
                               ...
                               WorldState *world_state)
{
        ...
        render_tile_map(
            image_buffer, (i32 *)world_state->next_tile_map->tile_map,
            world_state->tile_set, new_map_x_offset, new_map_y_offset);

        render_tile_map(
            image_buffer, (i32 *)world_state->current_tile_map->tile_map,
            world_state->tile_set, old_map_x_offset, old_map_y_offset);
